...
// "settings", "website", and "travelcal" are admin/permission-gated directly
// in renderNav and aren't something a manager can be granted piecemeal, so
// they're excluded from the staff permission checklist.
const PERMISSION_SECTIONS = NAV.filter((n) => n.id !== "dashboard" && n.id !== "settings" && n.id !== "website" && n.id !== "travelcal");
...
// B2B is a flat ₹10,000 off the standard rate — a fixed rule, not something
// set per package/tier.
const B2B_DISCOUNT = 10000;
const getB2bRate = (row) => row.b2cRate - B2B_DISCOUNT;
...
async function downloadRateCardPDF(name) {
  ...
  doc.setFontSize(12);
  doc.setTextColor(...PDF_COLORS.dark);
  doc.text(`Dear ${(name || "").split(" ")[0] || "there"},`, marginX, y);
  y += 7;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9.3);
  const intro = doc.splitTextToSize("Thank you for your interest in Together, Out Loud — here is our full rate card for your future reference, covering both our standard (B2C) and partner (B2B) pricing.", contentW);
  doc.text(intro, marginX, y);
  y += intro.length * 5 + 6;
  ...
  Object.values(byPackage).forEach((packageRows) => {
    ...
    doc.text("B2C", marginX + contentW * 0.55, y);
    doc.text("B2B", marginX + contentW * 0.8, y);
    y += 8;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.3);
    packageRows.forEach((row, i) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(inrPdf(row.b2cRate), marginX + contentW * 0.55, y);
      doc.setTextColor(...PDF_COLORS.rustDark);
      doc.text(inrPdf(getB2bRate(row)), marginX + contentW * 0.8, y);
      doc.setFont("helvetica", "normal");
      y += 7;
    });
    y += 6;
  ...
  y = pdfList(doc, [
    ...
    "Session duration will be 75 to 90 minutes (does not apply to Musical Pheras, which runs for the length of the ceremony).",
  ...
  const y2 = pdfList(doc, [
    ...
    "Rates on this card are indicative and subject to revision; final pricing is confirmed at the time of booking.",
        ...
        <div class="modal-body">
          ...
          <div id="tempArtistList">
            ${tempArtists.length === 0 ? `<p class="muted small">None added yet.</p>` : tempArtists.map((t) => {
              const waDigits = (t.phone || "").replace(/\D/g, "");
              const waMsg = `Hi ${t.name}, confirming your performance for ${lead.name} — ${packageName(lead.event_type)} on ${fmtDate(lead.date)}${lead.city ? ` in ${lead.city}` : ""}. Let us know if you have any questions!`;
              return `
              <div class="dash-list-item" style="display:flex; justify-content:space-between; align-items:center;">
                <div>
  ...
  function legCard(leg) {
    ...
    return `
      <div class="card" style="margin-bottom:10px;">
        ...
        ${(leg.departure_at || leg.arrival_at) ? `<div class="muted small" style="margin-top:6px;">${leg.departure_at ? `Departs ${fmtDateTime(leg.departure_at)}` : ""}${leg.departure_at && leg.arrival_at ? " · " : ""}${leg.arrival_at ? `Returns ${fmtDateTime(leg.arrival_at)}` : ""}</div>` : ""}
...
async function renderDocuments(main) {
  ...
  main.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      <p class="muted small" style="margin-top:-4px;">For contacts who haven't submitted an enquiry — event planners, agencies, or anyone wanting your rates for future reference. Shows both B2C and B2B pricing plus full terms &amp; conditions. No lead is created; this just builds a PDF and opens WhatsApp.</p>
      <div class="row-2">
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      <div class="table" style="margin-top:10px;">
        <div class="table-head" style="grid-template-columns:2fr 1fr 1fr 1fr;">
          <span>Package</span><span>Band size</span><span>B2C</span><span>B2B</span>
        </div>
        ${rateCardRows().map((row) => `
          <div class="table-row" style="grid-template-columns:2fr 1fr 1fr 1fr;">
            ...
            <span class="mono muted">${inr(getB2bRate(row))}</span>
          </div>
        `).join("")}
      </div>
    </div>
